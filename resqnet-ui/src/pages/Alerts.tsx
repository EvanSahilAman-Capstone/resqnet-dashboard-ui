      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs font-mono">
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-gray-700 font-semibold truncate">{incident.created_by}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
            ...
            {incident.coordinates && (
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-gray-700 font-semibold">[{incident.coordinates[0].toFixed(4)}, {incident.coordinates[1].toFixed(4)}]</p>
              </div>
            )}
            {incident.source_report_id && (
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-gray-400 uppercase tracking-wider mb-1">Source report</p>
                <p className="text-gray-700 font-semibold truncate">{incident.source_report_id}</p>
              </div>
            )}
          </div>

          {incident.reviewer_notes && (
            <p className="text-xs text-gray-500 italic border-l-2 border-gray-200 pl-3 py-1">{incident.reviewer_notes}</p>
          )}
        ...
        <div className="px-5 py-4 border-t bg-gray-50">
          ...
          <div className="flex gap-2 flex-wrap">
            {(["active","contained","resolved","closed"] as const)
              ...
              .map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg border text-xs font-semibold capitalize transition-colors disabled:opacity-50
                    ...
                    ${s === "closed"    ? "bg-gray-100 border-gray-200 text-gray-500 hover:bg-gray-200" : ""}
                  `}
                >
                  {updating ? <Loader2 size={11} className="animate-spin inline mr-1" /> : null}
                  Mark {s}
                </button>
              ))}
          </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-5 py-3 space-y-1 text-sm border-b bg-gray-50">
          <div className="flex items-center gap-2">
            ...
            <span className={`ml-auto px-2 py-0.5 rounded-full text-xs border ${SEV[report.severity]}`}>{report.severity.toUpperCase()}</span>
          </div>
        ...
        <div className="px-5 py-4 space-y-3 max-h-72 overflow-y-auto">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">Reviewer notes <span className="font-normal">(optional)</span></label>
            <textarea className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-300" rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add notes..." />
          </div>

          {/* Incident fields — pre-filled, used only if converting */}
          <div className="border border-orange-200 rounded-lg p-3 bg-orange-50 space-y-2">
            <p className="text-xs font-bold text-orange-600 uppercase tracking-wider">
              Incident Details <span className="font-normal text-orange-400">(used if converting to incident)</span>
            </p>
            <input className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm bg-white" placeholder="Title" value={incTitle} onChange={(e) => setIncTitle(e.target.value)} />
            <textarea className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm resize-none bg-white" rows={2} placeholder="Description" value={incDesc} onChange={(e) => setIncDesc(e.target.value)} />
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">Rejection reason <span className="font-normal text-gray-400">(required to reject)</span></label>
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-300" placeholder="e.g. Duplicate, insufficient evidence..." value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
          </div>
          {error && <p className="text-red-500 text-xs font-mono">{error}</p>}
        </div>

        <div className="px-5 py-4 border-t bg-gray-50 flex gap-2">
          <button disabled={loading} onClick={() => submit("verify")} className="flex-1 flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-2 rounded-lg disabled:opacity-50 transition-colors">
            <CheckCircle2 size={13} /> Verify
          </button>
          <button disabled={loading} onClick={() => submit("verify_and_change_to_incident")} className="flex-1 flex items-center justify-center gap-1.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold py-2 rounded-lg disabled:opacity-50 transition-colors">
            <Flame size={13} /> Verify + Incident
          </button>
          <button disabled={loading || !rejectReason.trim()} onClick={() => submit("reject")} className="flex-1 flex items-center justify-center gap-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-2 rounded-lg disabled:opacity-50 transition-colors">
...
function Alerts() {
  ...
  const handleAlertUpdate = async (id: string, updated: BroadcastAlert) => {
    try {
      const result = await fetchWithAuth(`/broadcasts/${id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: updated.message, radius: updated.radius,
          priority: updated.priority.toLowerCase(),
          coordinates: updated.position, timestamp: updated.timestamp,
        }),
      });
      setAlerts((prev) => prev.map((a) =>
        a.id === id ? { ...updated, id, updatedAt: result.broadcast?.updated_at ?? new Date().toISOString() } : a
      ));
  ...
  const handleIncidentStatusChange = (id: string, newStatus: string) => {
    setIncidents((prev) =>
      prev.map((inc) => inc.incident_id === id ? { ...inc, status: newStatus as Incident["status"] } : inc)
    );
  };
  ...
  const statusCounts = ["active","contained","resolved","closed"].reduce((acc, s) => {
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="mt-2 h-0.5 w-full bg-gray-100 rounded-full">
                <div className="h-0.5 bg-gray-400 rounded-full" style={{ width: "100%" }} />
              </div>
            </div>
            ...
            <div className="bg-white border border-red-200 rounded-xl p-5 shadow-sm">
              ...
              <p className="text-3xl font-bold text-red-600 tabular-nums">
                {incidentsLoading ? <span className="inline-block w-10 h-8 bg-red-50 animate-pulse rounded" /> : activeIncidents}
              </p>
              <div className="mt-2 h-0.5 w-full bg-red-100 rounded-full">
                <div className="h-0.5 bg-red-400 rounded-full transition-all duration-500"
            ...
            <div className="bg-white border border-amber-200 rounded-xl p-5 shadow-sm">
              ...
              <p className="text-3xl font-bold text-amber-600 tabular-nums">
                {alertsLoading ? <span className="inline-block w-10 h-8 bg-amber-50 animate-pulse rounded" /> : alerts.length}
              </p>
              {urgentAlerts > 0 && (
          ...
          {!incidentsLoading && incidents.length > 0 && (
            <div className="flex gap-1 mb-4 bg-gray-100 p-1 rounded-lg w-fit">
              {["all", "active", "contained", "resolved", "closed"].map((f) => (
                <button
                  key={f}
