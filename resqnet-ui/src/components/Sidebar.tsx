...
interface JwtUser {
  'https://resqnet.com/role'?: string;
  [key: string]: unknown;
}

const Sidebar: React.FC<SidebarProps> = ({ onLogoutClick }) => {
  const { user } = useAuth0();
  const role = (user as JwtUser)?.["https://resqnet.com/role"] ?? "Users";
  const isResponderOrAdmin = role === "Responder" || role === "Admin";

  return (
    <div className="flex h-screen w-16 flex-col justify-between border-r shrink-0 border-gray-100/50 bg-linear-to-b from-white/80 to-slate-50/80 shadow-xl backdrop-blur-sm z-40">
      {/* Logo */}
      <div className="pt-6">
      ...
      <div className="flex-1 flex flex-col space-y-2 px-2 py-6">
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `group relative flex justify-center p-3 rounded-xl hover:shadow-lg hover:scale-[1.05] transition-all duration-200 active:scale-[0.98]
             ${
               isActive
                 ? "bg-linear-to-r from-red-100/70 to-orange-100/70 text-red-700 border-red-200/60"
        ...
        >
          {({ isActive }) => (
            <>
              <HomeIcon
                className={`size-6 transition-opacity ${isActive ? "opacity-100" : "opacity-75 group-hover:opacity-100"}`}
              />
