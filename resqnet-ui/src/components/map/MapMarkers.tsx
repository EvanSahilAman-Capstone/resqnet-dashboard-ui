...
function safeMarker(
  ...
  if (!ok) {
    console.warn(
      `[MapMarkers] invalid coords ${label} id=${id} lng=${lng} lat=${lat} — skipped`,
    );
    return { valid: false };
  }
...
const FlameIcon = (c: string, s: number) =>
  Svg(
    <path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 3z" />,
    c,
    s,
  );
const RadioIcon = (c: string, s: number) =>
  Svg(
    <>
...
const CpuIcon = (c: string, s: number) =>
  Svg(
    <>
...
const NavIcon = (c: string, s: number) =>
  Svg(<path d="M3 11l19-9-9 19-2-8-8-2z" />, c, s, {
    transform: "rotate(-90deg)",
  });
const PinIcon = (c: string, s: number) =>
  Svg(
    <>
  ...
  return (
    <>
              ...
              <CirclePin
                ...
                onMouseEnter={() =>
                  onPopup({
                    ...
                    latitude: c.lat,
                    title: "Destination",
                    details: `[${c.lat.toFixed(3)}, ${c.lng.toFixed(3)}]`,
                  })
                }
                onMouseLeave={() => onPopup(null)}
              >
      ...
      {safeZones.map((zone) => {
        ...
        if (!c.valid) return null;
        const categoryLabel =
          SAFE_ZONE_CATEGORY_LABEL[zone.category] ?? "Safe Zone";
        return (
          <Marker
            key={`sz-${zone.safe_zone_id}`}
            longitude={c.lng}
            latitude={c.lat}
            anchor="center"
          >
            ...
            <FilledCirclePin
              ...
              onMouseEnter={() =>
                onPopup({
                  ...
                  latitude: c.lat,
                  title: inc.title,
                  details: `${inc.status.toUpperCase()} · ${inc.severity} · ${inc.category.replace(/_/g, " ")}`,
                })
              }
              onMouseLeave={() => onPopup(null)}
            >
      ...
      {fireReports.map((report) => {
        ...
        if (!c.valid || (report.status && report.status !== "pending_review"))
          return null;
        const color = SEV_COLOR[report.severity] ?? "#f97316";
        return (
          <Marker
            key={`fr-${report.report_id}`}
            longitude={c.lng}
            latitude={c.lat}
            anchor="center"
          >
            <CirclePin
              ...
              onMouseEnter={() =>
                onPopup({
                  ...
                  details: `By ${report.uploading_user} · ${report.severity} · pending review`,
                })
              }
              onMouseLeave={() => onPopup(null)}
            >
            ...
            <CirclePin
              ...
              onMouseEnter={() =>
                onPopup({
                  ...
                  latitude: c.lat,
                  title: fire.message,
                  details: `${fire.riskLevel} · [${c.lat.toFixed(3)}, ${c.lng.toFixed(3)}]`,
                })
              }
              onMouseLeave={() => onPopup(null)}
            >
            ...
            <BarePin
              ...
              onMouseEnter={() =>
                onPopup({
                  ...
                  latitude: c.lat,
                  title: alert.message,
                  details: `${alert.priority} · ${alert.radius > 0 ? alert.radius.toFixed(1) : "1.0"}km radius — click for details`,
                })
              }
              onMouseLeave={() => onPopup(null)}
            >
