...
const RV_COLOR = 6, RV_OPTIONS = '1_0', RV_SIZE = 512;
...
const MapLayers: React.FC<MapLayersProps> = ({
  ...
  useEffect(() => {
    if (!t?.liveWeather) { setRadarTileUrl(null); return; }
    let cancelled = false;
    const fetchRadar = async () => {
      ...
      } catch (err) { console.error('[ResQNet] RainViewer fetch failed:', err); }
    };
    fetchRadar();
  ...
  const [customGeoJSON, setCustomGeoJSON] = useState<FeatureCollection | null>(null);

  useEffect(() => {
    if (!customOverlayFile) { setCustomGeoJSON(null); return; }
    const reader = new FileReader();
    reader.onload = (e) => {
      try { setCustomGeoJSON(JSON.parse(e.target?.result as string)); }
      catch { console.error('Invalid GeoJSON file'); }
    };
  ...
  const sensorCoverageCollection: FeatureCollection = {
    ...
    features: sensors.map((s) =>
      circle([s.longitude, s.latitude], SENSOR_RADIUS_KM, {
        steps: 64, units: 'kilometers',
        properties: {
          color: s.status === 'ONLINE' ? '#10b981' : s.status === 'WARNING' ? '#f59e0b' : '#6b7280',
        },
      }) as Feature
    ),
  };
  ...
  const fireHeatmapCollection: FeatureCollection = {
    ...
    features: fires.map((f) => ({
      type: 'Feature',
      properties: {
        weight: f.riskLevel === 'EXTREME' ? 1 : f.riskLevel === 'HIGH' ? 0.75
              : f.riskLevel === 'MEDIUM'  ? 0.5 : 0.25,
      },
  ...
  const evacuationZoneCollection: FeatureCollection = {
    ...
    features: fires
      ...
      .map((f) => circle([f.longitude, f.latitude], 10, {
        steps: 32, units: 'kilometers',
        properties: { color: SEVERITY_COLORS[f.riskLevel.toLowerCase()] ?? '#DC2626' },
      }) as Feature),
  };
  ...
  const heatRadiusCollection: FeatureCollection = {
    ...
    features: fires
      .filter((f) => f.riskLevel === 'HIGH' || f.riskLevel === 'EXTREME')
      .map((f) => circle([f.longitude, f.latitude], 2, {
        steps: 64, units: 'kilometers',
    ...
    <>
      ...
      {t?.sensorCoverage && sensors.length > 0 && (
        <Source id="sensor-coverage" type="geojson" data={sensorCoverageCollection}>
          ...
          <Layer id="sensor-coverage-outline" type="line"
          ...
          <Layer id="fire-heatmap-layer" type="heatmap"
            paint={{
              ...
              'heatmap-color': [
                ...
                0, 'rgba(0,0,255,0)', 0.2, '#FCD34D', 0.4, '#FB923C',
                0.6, '#EF4444', 0.8, '#DC2626', 1, '#7F1D1D',
              ],
              'heatmap-radius':  ['interpolate', ['linear'], ['zoom'], 0, 20, 12, 50],
      ...
      {t?.evacuationZones && evacuationZoneCollection.features.length > 0 && (
        <Source id="evacuation-zones" type="geojson" data={evacuationZoneCollection}>
          ...
          <Layer id="evacuation-zones-outline" type="line"
      ...
      {t?.heatRadius && heatRadiusCollection.features.length > 0 && (
        <Source id="heat-radius" type="geojson" data={heatRadiusCollection}>
          ...
          <Layer id="heat-radius-outline" type="line"
      ...
      {t?.liveWeather && radarTileUrl && (
        <Source id="rainviewer-radar" type="raster" tiles={[radarTileUrl]}
          ...
          <Layer id="rainviewer-radar-layer" type="raster"
      ...
      {customGeoJSON && (
        <Source id="custom-overlay" type="geojson" data={customGeoJSON}>
          ...
          <Layer id="custom-overlay-outline" type="line"
