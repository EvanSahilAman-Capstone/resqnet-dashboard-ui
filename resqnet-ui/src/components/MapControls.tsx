...
import { Flame, Radio, Compass, List, Plus, ChevronRight, Settings } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="absolute top-4 right-4 z-50 flex items-center gap-1 bg-white/95 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200 px-2 py-1.5">
        ...
        {/* Incidents */}
        <button type="button" onClick={() => setIncidentsOpen(!incidentsOpen)} title="Incidents"
        ...
        <div className="relative" ref={menuRef}>
          <button type="button" onClick={() => setShowBroadcastMenu((p) => !p)} title="Broadcast"
            className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all text-xs font-medium
        ...
        {/* Legend */}
        <button type="button" onClick={() => togglePanel("legend")} title="Legend"
          className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all text-xs font-medium
        ...
        {/* Layers / Settings */}
        <button type="button" onClick={() => togglePanel("layers")} title="Map Layers"
          className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all text-xs font-medium
            ${openPanels.has("layers") ? "bg-gray-100 text-gray-800" : "text-gray-600 hover:bg-gray-100 hover:text-gray-800"}`}>
          <Settings size={18} />
