...
const MapComponent: React.FC<MapProps> = ({
  ...
  useEffect(() => {
    if (!isPlacingAlert) setMousePosition(null);
  }, [isPlacingAlert]);
  ...
  const draftAlertCircleFeature: Feature<Polygon> | null = mousePosition
    ? (circle(mousePosition, draftRadiusKm, {
        steps: 64,
        units: 'kilometers',
        properties: { color: priorityColors[draftPriority] },
      }) as Feature<Polygon>)
    : null;

  const draftAlertCollection: FeatureCollection | null = draftAlertCircleFeature
    ? { type: 'FeatureCollection', features: [draftAlertCircleFeature] }
    : null;
        ...
        onMouseMove={(e) => {
          ...
          if (routeGeoJSON && mapRef.current) {
            ...
            if (features.length > 0) {
              ...
              if (popupInfo?.title === 'Evacuation Route') setPopupInfo(null);
            }
          }
        }}
        onClick={(e) => {
      ...
      >
        ...
        {fires.map((fire) => (
          <Marker key={fire.id} longitude={fire.longitude} latitude={fire.latitude}>
            <img src={reportGif} style={{ width: 40, height: 40, cursor: 'pointer' }} alt="Fire"
              onMouseEnter={() =>
                setPopupInfo({
        ...
        {sensors.map((sensor) => (
          <Marker key={sensor.id} longitude={sensor.longitude} latitude={sensor.latitude}>
            <img src={sensorGif} style={{ width: 40, height: 40, cursor: 'pointer' }} alt="Sensor"
              onMouseEnter={() =>
                setPopupInfo({
        ...
        {broadcastAlerts.map((alert) => {
          ...
          return (
            <Marker key={alert.id} longitude={alert.position[1]} latitude={alert.position[0]}>
              <img src={alertGif} style={{ width: 40, height: 40, cursor: 'pointer' }} alt="Alert"
                onMouseEnter={() =>
                  setPopupInfo({
        ...
        {popupInfo && (
          <Popup
            longitude={popupInfo.longitude} latitude={popupInfo.latitude}
            anchor="top" closeButton={false} closeOnClick={false}
            onClose={() => setPopupInfo(null)} offset={20} className="custom-popup"
          >
            <div
