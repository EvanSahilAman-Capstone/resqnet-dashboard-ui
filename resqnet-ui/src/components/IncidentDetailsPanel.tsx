        ...
        <div key={d.label} className="flex items-center gap-2">
          ...
          <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.round((d.value / peak) * 100)}%`,
                background: d.color,
              }}
            />
          </div>
...
const EditField: React.FC<{
  ...
  return (
    <div className="flex items-start gap-1.5">
      ...
      <button
        onClick={commit}
        className="p-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white mt-0.5"
      >
        <Check size={12} />
      </button>
      <button
        onClick={cancel}
        className="p-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-600 mt-0.5"
      >
        <X size={12} />
      </button>
    </div>
...
const SectionHead: React.FC<{ icon: React.ReactNode; label: string }> = ({
  icon,
  label,
}) => (
...
const IncidentDetailsPanel: React.FC<Props> = ({
  incident: inc,
  onClose,
  onFlyTo,
  onUpdated,
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "logs" | "comments">(
    "overview",
  );
    ...
    <div className="fixed top-0 right-0 h-full w-[380px] bg-white border-l border-gray-200 shadow-2xl z-[9999] flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          <span
            className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${SEV_STYLE[inc.severity]}`}
          >
            {inc.severity.toUpperCase()}
          </span>
          <span
            className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_STYLE[fields.status]}`}
          >
            {fields.status}
          </span>
          <button
      ...
      <div className="flex border-b border-gray-100 shrink-0">
        {(["overview", "logs", "comments"] as const).map((tab) => (
          ...
          >
            ...
            {tab === "comments" && inc.comments?.length
              ? ` (${inc.comments.length})`
              : ""}
          </button>
        ))}
              ...
              <div className="grid grid-cols-2 gap-3 mt-1">
                <Row icon={<AlertTriangle size={13} />} label="Status">
                  <EditField
                    ...
                    display={
                      <span
                        className={`text-[11px] font-semibold px-2 py-0.5 rounded-full inline-block ${STATUS_STYLE[fields.status]}`}
                      >
                        {fields.status}
                      </span>
                    }
              ...
              {hasCoords ? (
                <div className="flex items-center justify-between bg-gray-50 rounded-xl px-3 py-2">
                  <span className="text-[11px] font-mono text-gray-600">
                    {inc.coordinates[0].toFixed(5)},{" "}
                    {inc.coordinates[1].toFixed(5)}
                  </span>
                  {onFlyTo && (
                    <button
                      onClick={() =>
                        onFlyTo(inc.coordinates[0], inc.coordinates[1])
                      }
