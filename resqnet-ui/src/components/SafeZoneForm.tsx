...
import { ShieldCheck, ChevronUp, ChevronDown, RotateCcw, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        />
        {nameError && (
          <p className="text-xs text-red-500 mt-1">Name required before placing.</p>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-gray-400 uppercase tracking-wider font-medium shrink-0">Type</span>
        <select
          value={form.category}
          onChange={(e) => update({ category: e.target.value as SafeZoneFormData['category'] })}
      ...
      {/* Radius */}
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-gray-400 uppercase tracking-wider font-medium shrink-0">Radius</span>
        <input
          type="range"
          min="50"
          max="5000"
          ...
          {(['active', 'at_capacity', 'closed'] as const).map((s) => (
            <button
              ...
              className={`text-[10px] font-semibold px-2 py-1 rounded-full border transition-all
                ${form.status === s
                  ? s === 'active'      ? 'bg-green-100 text-green-800 border-green-400 ring-2 ring-green-300'
                  : s === 'at_capacity' ? 'bg-yellow-100 text-yellow-800 border-yellow-400 ring-2 ring-yellow-300'
      ...
      <div className="flex gap-2 pt-1">
        ...
        >
          <ShieldCheck size={15} className={isPlacingSafeZone ? 'animate-pulse' : ''} />
          <span>
            {loading ? 'Creating...' : isPlacingSafeZone ? 'Placing...' : 'Place on Map'}
          </span>
        </button>
      </div>
    </div>
